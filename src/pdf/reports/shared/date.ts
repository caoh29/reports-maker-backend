import { Content } from 'pdfmake/interfaces';
import { DateFormatter } from 'src/lib/helpers/date-formatter';
import { COLOR_YELLOW_HEX } from 'src/lib/constants';

export const getDate = (date?: Date | string): Content => {
  let validDate: Date | undefined;

  if (typeof date === 'string') {
    validDate = new Date(date);
  } else if (date instanceof Date) {
    validDate = date;
  }

  const isValid = validDate instanceof Date && !isNaN(validDate.getTime());

  const dateContent: Content = {
    text: isValid ? DateFormatter.getDDMMYYYY(validDate!) : '[Date]',
    alignment: 'left',
    margin: [0, 150, 0, 40],
    background: date ? undefined : COLOR_YELLOW_HEX,
  };

  return {
    columns: [dateContent],
  };
};
